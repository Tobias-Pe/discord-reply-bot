...
func populateChoices(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	case toBeAnsweredOptionName:
		choices = populateChoicesForToBeAnswered(focusedDataOption, choices, nameToOptionMap)
	default:
...
func populateChoicesForToBeAnswered(focusedDataOption *discordgo.ApplicationCommandInteractionDataOption, choices []*discordgo.ApplicationCommandOptionChoice, nameToOptionMap map[string]*discordgo.ApplicationCommandInteractionDataOption) []*discordgo.ApplicationCommandOptionChoice {
	...
	toBeMatchedOption := nameToOptionMap[toBeMatchedOptionName]
	if matchTypeOption == nil || toBeMatchedOption == nil {
		logger.Logger.Debugw("Couldn't fetch value of option match-type and/or to-be-matched", "Map", nameToOptionMap)
		return choices
	}
	...
	selectedMatchChoices := commands.SearchChoices(focusedDataOption.StringValue(), values)
